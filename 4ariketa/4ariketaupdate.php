...
<body> <!-- Body etiketa irekiten du -->
    ...
    <form method="post"> <!-- Formulario bat sortu du datuak editatzeko balioko duena post metodoa erabiliz -->
        ...
        <select id="mota" name="mota" required> <!-- Selekt bat sortu du motaren ezberdin guztiak izango dituen desplegablea izateko -->
            <!-- motaren balio bakoitzerako konprobazio bat sortu da defektuz aukeratuta egon dedin produktuak zuen mota -->
            <option value="Portatil" <?php if ($mota == 'Portatil') // Portatila bada portatil balorea aterako da defektuz
                echo 'selected'; ?>>Portatil</option>
            <option value="Kontsola" <?php if ($mota == 'Kontsola') // Kontsola bada kontsola balorea aterako da defektuz
                echo 'selected'; ?>>Kontsola</option>
            <option value="Periferiko" <?php if ($mota == 'Periferiko') // Periferikoa bada periferiko balorea aterako da defektuz
                echo 'selected'; ?>>Periferiko</option>
            <option value="Cascos" <?php if ($mota == 'Cascos') // Kaskoak bada casco balorea aterako da defektuz
    ...
    if (isset($_POST['editatubotoia'])) { // Botoiari sakatu dion kondizioa sortzen du
        ...
        if ($conn->query($sql) === TRUE) { // SQL kontsulta ongi egin den egiaztatzen duen kondizioa
            header("Location: 4ariketa.php"); // Ongi egin badu, aurreko lehiora bueltatuko da
